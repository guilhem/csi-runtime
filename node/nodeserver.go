...
var ErrVolumeNotReady = errors.New("volume is not yet ready to be setup")

func New(nodeID string, maxVolumesPerNode int64, manager Interface, store storage.Interface, log logr.Logger) (*Server, error) {
...
func (ns *Server) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	if err := ns.manager.ValidateCapability(volumeCapability); err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "VolumeCapability is invalid: %v", err)
	}
